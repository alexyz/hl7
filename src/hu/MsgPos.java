...
public class MsgPos implements Comparable<MsgPos> {
	...
	public MsgPos (int segOrd, int fieldOrd, int fieldRep, int compOrd, int subCompOrd) {
		if (!(segOrd >= 1 || fieldOrd >= 0 || fieldRep >= 0 || compOrd >= 1 || subCompOrd >= 1)) {
			throw new RuntimeException(String.format("invalid position: seg %d >= 1 field %d >= 0 rep %d >= 0 comp %d >= 1 subcomp %d >= 1", 
					segOrd, fieldOrd, fieldRep, compOrd, subCompOrd));
		}
		this.segOrd = segOrd;
